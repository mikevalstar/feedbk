...
export function LibraryProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo(() => {
    const cart = cartIds.map((id) => BOOKS.find((book) => book.id === id)).filter((book): book is Book => !!book);
    return {
      books: BOOKS,
      cart,
      addToCart: (book: Book) => setCartIds((prev) => (prev.includes(book.id) ? prev : [...prev, book.id])),
